...
then
	...
	sed -n "$endofmain,$size p" test-printf/test-printf.c | grep -e "return" -e "\[" | tr -d '\t' | sed 's/%/%%/g' | sed 's/"/\\"/g' | sed 's/return(ac == 2 ? printf(.*) : ft_printf/test "$i - /' | sed 's/);/ : ${NC}" $i /' | sed 's/\/\/ \[/printf "\\n\\n${BLUE}\[/g' | sed 's/\]/\]${NC}\\n" \&\& error=0/g' >> test-printf/runtest-printf.sh
fi
